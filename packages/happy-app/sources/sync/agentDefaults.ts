...
export const agentKeys = ['claude', 'codex', 'gemini', 'openclaw', 'agy', 'pi'] as const;
...
const codeAgentDefaults: Record<AgentKey, AgentDefaultConfig> = {
    ...
    // Pi reads its own model/auth config; Happy does not forward a default.
...
export function normalizeAgentKey(flavor: string | null | undefined): AgentKey {
    if (flavor === 'codex' || flavor === 'gemini' || flavor === 'openclaw' || flavor === 'agy' || flavor === 'pi') {
        return flavor;
    }
    return 'claude';
}
