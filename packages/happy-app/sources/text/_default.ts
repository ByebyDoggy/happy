...
export const en = {
    ...
    sessionCategories: {
        ...
        notLoaded: 'Categories are still loading. Check your connection and try again.',
    ...
    sidebar: {
        ...
        projects: 'Projects',
        selectArchived: 'Select',
        cancelSelection: 'Cancel',
        selectAllArchived: 'Select all',
        deselectAllArchived: 'Deselect all',
        deleteSelected: ({ count }: { count: number }) => `Delete ${plural({ count, singular: 'session', plural: 'sessions' })}`,
        deleteArchivedTitle: 'Delete archived sessions',
        deleteArchivedMessage: ({ count }: { count: number }) => `Permanently delete ${plural({ count, singular: 'this session', plural: `${count} archived sessions` })}? The conversations, messages, and attachments are removed from the server and cannot be recovered.`,
        deleteArchivedBotsKept: ({ count }: { count: number }) => `${plural({ count, singular: 'A bot was', plural: `${count} bots were` })} left alone — a bot keeps one continuous conversation, so archive it instead.`,
        deleteArchivedAllBots: ({ count }: { count: number }) => `${plural({ count, singular: 'The selected session is a bot', plural: `All ${count} selected sessions are bots` })}. A bot keeps one continuous conversation and cannot be deleted — archive it instead.`,
        deleteArchivedPartialTitle: 'Some sessions were not deleted',
        deleteArchivedPartialMessage: ({ deleted, failed }: { deleted: number; failed: number }) => `Deleted ${deleted}; ${failed} failed.`,
    },
    ...
    onboarding: {
        ...
        restoreOr: 'or',
        enterSecretKey: 'Enter a secret key',
        secretKeyMigrationNote: 'Both devices stay signed in and stay in sync. Nothing is removed from the other device.',
